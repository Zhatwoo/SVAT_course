...
export default function AdminDashboardPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(
          () => reject(new Error("Loading timed out. Please refresh the page.")),
          15000,
        );
      });

      const [courseData, chapterData, episodeData, settings] = await Promise.race([
        dataPromise,
        timeoutPromise,
      ]);
  ...
  if (loadError) {
    return (
      <div className="admin-page flex min-h-screen flex-col items-center justify-center gap-4 bg-background px-6 text-center">
